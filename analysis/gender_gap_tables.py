...
# ───────────────────────── file paths ───────────────────────────
TT_FILE  = Path("outputs/country_output.csv")            # use filtered TikTok file
...
NAME2ISO = {
    _normal(n): c for n,c in zip(pop["country_name"], pop["country_code"])
}

# -------------------------------------------------------------------
#  STEP B · read filtered TikTok file
# -------------------------------------------------------------------
aud = pd.read_csv(TT_FILE)

# map age_bucket to our numeric buckets
aud["bucket"] = aud["ages_ranges"].map(RANGE_MAP)
if aud["bucket"].isna().any():
    raise ValueError(
        "Unknown age_bucket labels: "
        f"{aud.loc[aud['bucket'].isna(),'ages_ranges'].unique()}"
    )

# -------------------------------------------------------------------
#  STEP C · ensure country_code is present
# -------------------------------------------------------------------
if "country_code" not in aud.columns:
    raise KeyError("Expected 'country_code' column in country_output.csv")

# -------------------------------------------------------------------
#  STEP D · tidy TikTok audience
# -------------------------------------------------------------------
# if est_users not already computed, compute from lower/upper
...
if merged.empty:
    raise RuntimeError("Merged table is empty – check country codes & bucket labels")
