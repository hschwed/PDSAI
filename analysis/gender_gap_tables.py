...
# ─────────────────── 2. region_id → ISO-3 map ───────────────────
ref = pd.read_csv(REF_FILE)
id2iso = (
    ref.loc[ref["region_level"].str.upper() == "COUNTRY",
...
# add est_users & sex if absent
if "est_users" not in aud.columns:
    aud["est_users"] = (aud["lower_end"] + aud["upper_end"]) / 2

if "sex" not in aud.columns:
    aud["sex"] = aud["genders"].map({
        "GENDER_MALE":   "male",
        "GENDER_FEMALE": "female"
    })

# add country_code via geo_location
if "country_code" not in aud.columns:
    if "geo_location" not in aud.columns:
        sys.exit("❌ country_output.csv lacks geo_location column")
    aud = aud.merge(id2iso, left_on="geo_location", right_on="region_id", how="inner")
...
if len(bad):
    sys.exit(f"❌ Unknown age ranges: {bad}")
...
if merged.empty:
    sys.exit("❌ Merge produced zero rows (check country codes & buckets)")
...
by_cty[["gap_abs", "gap_pct"]] = \
    by_cty[["gap_abs", "gap_pct"]].round(4)
